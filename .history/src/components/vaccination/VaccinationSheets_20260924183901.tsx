...
export function VaccinationCompleteSheet({ p, onClose }: { p: VaccinationPosition; onClose: () => void }) {
  ...
  // Labour and vaccinator charges — separate FinanceTxn rows in the Finance ledger.
  const [labour, setLabour] = useState({ amount: '', vaccinatorAmount: '', counterparty: '' });
  ...
  return (
    <Dialog open onClose={onClose} title="Mark vaccination done"
      ...
      <div className="space-y-3">
        ...
        {/* Labour and vaccinator charges — separate Finance ledger rows */}
        <div className="rounded-[16px] border border-line bg-card p-3.5 space-y-3">
          <div>
            <p className="font-mono text-[9.5px] font-semibold uppercase tracking-[0.14em] text-muted">
              Vaccination charges
            ...
            <>
              ...
              <div className="rounded-[10px] bg-sunk px-3 py-2">
                <p className="font-mono text-[10px] text-muted leading-relaxed">
                  ...
                  {labourAmt + vaccinatorAmt > 0 && <span className="block font-semibold text-ink-2 mt-0.5">Total vaccine charges: {fmtMoney(labourAmt + vaccinatorAmt)}</span>}
                </p>
              </div>
            </>
          )}
