  ...
  (set, get) => {
    ...
    return {
      ...
      completeVaccination: (id, input) => {
        ...
        // Vaccine Labour and Vaccinator charges become real Finance ledger rows so they
        // appear in the shed's P&L, the batch P&L and every financial report, exactly like
        // any other expense. They are separate rows — never merged — so ledger and
        // reporting can identify each one. The `refId` ties them to this vaccination so
        // a future edit/reversal can find and replace them.
        const companyId = cid() ?? item.companyId;
        const createdBy = get().session?.userId ?? 'system';
        const labourRows: FinanceTxn[] = [];
        if (input.labourAmount && input.labourAmount > 0) {
          labourRows.push({
            id: uid('fx'), companyId, batchId: item.batchId,
            date,
            kind: 'EXPENSE',
        ...
        set(s => ({
          ...
          finance: labourRows.length ? [...s.finance, ...labourRows] : s.finance,
