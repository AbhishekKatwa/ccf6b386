...
| `010_owner_user_creation.sql` | an OWNER may create a person in their own company: `create_login` grows to write profiles + company_users in one guarded transaction, and profiles gets one manager-passable INSERT policy behind an append-only trigger |
| `011_ref_id_linkage.sql` | `finance_txns.ref_id` loses its sale_entries foreign key: the vaccine flow stamps its own `vac-*` link there, and like `purchase_id` the column is app-level bookkeeping that `replaceLedger()` and `deleteSaleEntry()` already handle |
| `012_identity_id.sql` | `create_login` gives the new identity its own `gen_random_uuid()` instead of the caller's uuid, and re-keys the rows the old stamp left: one admin's second "create user" no longer dies on `identities_pkey` |
| `013_company_lifecycle.sql` | *written, not yet applied* — company ACTIVE/INACTIVE enforced inside the RLS helpers, so a suspended company stops reading and writing as one rule instead of one policy per table |
| `014_shed_geometry.sql` | the farm site plan: six nullable numeric columns on `sheds` (position, heading, metres) and a plot size on `farms`, all range-checked. The layout editor keeps its own defaults when they are null, so nothing needed back-filling |
