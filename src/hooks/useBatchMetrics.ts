...
export function useBatchMetrics(batchId: string | undefined) {
  const { batches, mortality, feed, eggs, saleEntries, eggWastages, finance, medicineStock } = useCompanyData();
  ...
  return useMemo(() => {
    ...
    const stock = eggStockTrays(batch.shedId, eggs, saleEntries, eggWastages, today);
  ...
  }, [batchId, batches, mortality, feed, eggs, saleEntries, eggWastages, finance, medicineStock]);
}
