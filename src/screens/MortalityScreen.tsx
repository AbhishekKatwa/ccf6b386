...
import { Skull, Plus, Lock } from 'lucide-react';
...
export function MortalityScreen() {
  ...
  const isLocked = (d: string) => dayLocks.some(l => l.batchId === batch.id && l.date === d);
  ...
  function submit() {
    ...
    const r = addMortality({ batchId: batch!.id, date: form.date, count: c, remarks: form.remarks || undefined });
  ...
  function submitLock() {
    const r = lockDay(batch!.id, lockForm.date, lockForm.reason || undefined);
    if (!r.ok) return pushToast('error', r.error ?? 'Failed');
    ...
    <Page withNav>
      ...
      <div className="px-4 sm:px-0 mt-3 space-y-4">
        ...
        <div className="flex gap-2">
          {canCreate && <Button block variant="primary" icon={<Plus size={15} />} onClick={() => setOpen(true)}>Add entry</Button>}
          {canLock && <Button block variant="outline" icon={<Lock size={15} />} onClick={() => setLockOpen(true)}>Lock day</Button>}
        </div>
