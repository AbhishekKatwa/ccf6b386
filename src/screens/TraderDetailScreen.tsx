...
export function TraderDetailScreen() {
  ...
  const totals = list.reduce((acc, t) => {
    if (t.kind === 'EGG_SALE') { acc.sales += t.amount; acc.trays += t.trays ?? 0; }
    if (t.kind === 'PAYMENT_IN') acc.payments += t.amount;
    return acc;
  }, { sales: 0, payments: 0, trays: 0 });
    ...
    <Page withNav>
      ...
      <div className="px-4 sm:px-0 mt-3 space-y-4">
        <Card>
          ...
          <Row label="Total trays" value={fmtIN(totals.trays)} />
        </Card>
        ...
        {canManage && <Button block variant="accent" icon={<Plus size={15} />} onClick={() => setOpen(true)}>Add transaction</Button>}
                  ...
                  <div key={t.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-mono text-[11px] text-muted truncate mt-0.5 tnum">
                        {fmtDate(t.date)}{t.trays ? ` · ${fmtIN(t.trays)} trays` : ''}{t.rate ? ` @ ${canFinance ? fmtMoney(t.rate, 2) : '₹••'}` : ''}
                      </p>
                    </div>
        ...
        <div className="space-y-3">
          ...
          {form.kind === 'EGG_SALE' && (
            <div className="grid grid-cols-2 gap-3">
              <Field label="Trays" type="number" inputMode="numeric" value={form.trays} onChange={e => setForm(f => ({ ...f, trays: e.target.value }))} className="font-mono" />
              <Field label="Rate / tray (₹)" type="number" inputMode="decimal" step="0.01" value={form.rate} onChange={e => setForm(f => ({ ...f, rate: e.target.value }))} className="font-mono" />
            </div>
          )}
          ...
          {form.kind === 'RATE_UPDATE' && (
            <Field label="New rate (₹/tray)" type="number" inputMode="decimal" step="0.01" value={form.rate} onChange={e => setForm(f => ({ ...f, rate: e.target.value }))} className="font-mono" />
          )}
